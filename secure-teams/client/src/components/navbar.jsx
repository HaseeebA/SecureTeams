...
const Navbar = ({ selectedTheme }) => {
    ...
    const [invertImages, setInvertImages] = useState(false);
    const noHoverColors = ["#0b1623", "#540D0D", "#013220"];
    const role = localStorage.getItem("role") || "Employee";  // Retrieve role directly
    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            try {
                ...
                const { email, name, profilePhoto } = response.data;
                if (profilePhoto) {
                    const profilePhotoUrl = apiBaseUrl + "/uploads/" + profilePhoto;
                    console.log("Profile Photo URL", profilePhotoUrl);
                    setProfilePhoto(profilePhotoUrl);
                }
            } catch (error) {
    ...
    return (
        <div className="navbar" style={{ backgroundColor: selectedTheme }}>
            ...
            <div className="relative inline-block">
                <img src={profilePhoto || profile} alt="Profile" className="profile-button w-10 h-10 rounded-full cursor-pointer" onClick={handleProfileClick} />
                {isProfileOpen && (
                    <div className="profile-dropdown show">
                        <button className="block w-full px-4 py-2 text-left hover:bg-gray-200">
                            <NavLink to="/profile">Profile</NavLink>
